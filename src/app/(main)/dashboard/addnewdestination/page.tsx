...
import { FaTimes, FaImage, FaGlobe, FaSearch, FaSpinner, FaPlus } from "react-icons/fa";
...
interface DestinationFormData {
  ...
  associatedTrip: string; // Changed from array to single string
  tags: Set<string>;
}
...
interface Trip {
  $id: string;
  name: string;
  title?: string; // Add title field for compatibility
  slug: string;
  featuredImage?: string;
  tags?: string[];
  difficulty?: string;
...
export default function AddNewDestinationPage() {
  ...
  const [formData, setFormData] = useState<DestinationFormData>({
    ...
    associatedTrip: "", // Changed from array to single string
    tags: new Set(),
  });

  // Trips state for dropdown
  const [trips, setTrips] = useState<Trip[]>([]);
  const [loadingTrips, setLoadingTrips] = useState(false);

  // Prefetch common routes for faster navigation
  useEffect(() => {
    router.prefetch('/dashboard/alldestinations');
  ...
  // Auto-refresh trips if we have selected trip but no trip data
  useEffect(() => {
    if (formData.associatedTrip && trips.length === 0 && !loadingTrips) {
      console.log('Auto-refreshing trips because we have selected trip but no trip data');
      fetchTrips();
    }
  }, [formData.associatedTrip, trips.length, loadingTrips]);
  ...
  const fetchTrips = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.log('First trip structure:', fetchedTrips[0]);
        console.log('First trip name:', fetchedTrips[0]?.name);
        console.log('First trip ID:', fetchedTrips[0]?.$id);
        
        setTrips(fetchedTrips);
        if (fetchedTrips.length === 0) {
          console.log("No trips available in the collection");
        }
      } else {
    ...
    try {
      const response = await fetch('/api/destinations/create', {
        ...
        body: JSON.stringify({
          ...
          featuredImageBucket: formData.featuredImageBucket,
          associatedTrip: formData.associatedTrip,
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.title || !formData.metaDescription || !formData.selectedImageId || !user) {
      setError("Please fill in all required fields and select a featured image");
      return;
    }
    ...
    try {
      ...
      router.push('/dashboard/alldestinations');
    } catch (error: unknown) {
    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <Button 
          ...
          onClick={() => {
            setNavigating(true);
            router.replace('/dashboard/alldestinations', { scroll: false });
          }}
        ...
        <CardContent>
          <form onSubmit={submit} className="space-y-6">
            ...
            {/* Associated Trip */}
            <div className="space-y-2">
              <Label htmlFor="associatedTrip">Associated Trip</Label>
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    value={formData.associatedTrip}
                    onChange={(e) => {
                      const selectedTrip = trips.find(t => t.$id === e.target.value);
                      console.log('Selected trip:', selectedTrip);
                      setFormData(prev => ({
                        ...prev,
                        associatedTrip: e.target.value
                      }));
                    }}
                  ...
                  >
                    ...
                    {trips.map((trip) => (
                      <option key={trip.$id} value={trip.$id}>
                        {trip.name || trip.title || `Trip ${trip.$id.substring(0, 8)}...`}
                      </option>
                    ))}
                  </select>
                  <Button
                ...
                {formData.associatedTrip && (
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(() => {
                        const trip = trips.find(t => t.$id === formData.associatedTrip);
                        console.log('Rendering selected trip badge:', { 
                          tripId: formData.associatedTrip, 
                          trip, 
                          tripName: trip?.name,
                        ...
                        // Try to get the trip name from various possible fields
                        const tripName = trip?.name || trip?.title || `Loading... (${formData.associatedTrip.substring(0, 8)}...)`;
                        
                        return (
                          <Badge 
                            variant="secondary" 
                            className="flex items-center gap-2 px-3 py-1 bg-indigo-100 text-indigo-800 border border-indigo-200 hover:bg-indigo-200 transition-colors"
                          >
                ...
                <p className="text-xs text-gray-500">
                  Select a trip to associate with this destination (optional)
                </p>
                  ...
                  <div className="mt-2 space-y-2">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        console.log('Current selected trip:', formData.associatedTrip);
                ...
                {process.env.NODE_ENV === 'development' && (
                  <div className="mt-2 p-2 bg-gray-100 rounded text-xs">
                    ...
                    Available trips: {trips.length}<br/>
                    Selected trip ID: {formData.associatedTrip || 'None'}<br/>
                    Loading: {loadingTrips ? 'Yes' : 'No'}<br/>
                    {trips.length > 0 && (
                      <>
                        First trip ID: {trips[0]?.$id}<br/>
                        First trip name: {trips[0]?.name || 'NO NAME'}<br/>
                        First trip title: {trips[0]?.title || 'NO TITLE'}<br/>
                        All trip names: {trips.map(t => t.name || 'NO NAME').join(', ')}
                      </>
                    )}
                  </div>
                )}
              ...
              {loadingMedia ? (
                ...
                <div className="text-center py-8">
                  ...
                  {filteredMedia.map((file) => (
                      <div
                      key={file.id}
                        className="relative group cursor-pointer border rounded-lg overflow-hidden hover:border-indigo-500 transition-colors"
